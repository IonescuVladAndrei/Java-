...
public class ImageFilterNegative extends Thread implements ImageFilter {
    ...
    public ImageFilterNegative(BufferedImage buffImage, ImagePaths imagePaths, int segmentVlaue, ImageView imageView) {
    ...
    public void run() {
        ...
        try {
            ...
            if (this.segmentVlaue == 16) {
                while (ImageFilterNegative.nrOfThreadsDone < 15)
                    try {
                        Thread.sleep(10);
                    } catch (InterruptedException e) {
                        System.out.println("Error: Thread.sleep(10) (ImageFilterNegative)");
                        e.printStackTrace();
                    }
                end = (int) System.currentTimeMillis();
                ImageWriterClass imageWriterClass = new ImageWriterClass(imagePaths, buffImage, end - start, "ImageFilterNegative", "-", imageView);
                imageWriterClass.writeImage();
            }
            end = (int) System.currentTimeMillis();
            this.setExecTime(end - start);
            ImageFilterNegative.nrOfThreadsDone++;
        } catch (IOException e) {
