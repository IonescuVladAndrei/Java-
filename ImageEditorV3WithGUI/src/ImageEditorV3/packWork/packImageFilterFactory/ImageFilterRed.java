...
public class ImageFilterRed extends Thread implements ImageFilter {
    ...
    public ImageFilterRed(BufferedImage buffImage, ImagePaths imagePaths, int segmentVlaue, int inputFilterValue, ImageView imageView) {
        ...
        try {
            ...
            for (i = imageSize.widthStart(); i <= imageSize.widthEnd(); i++)
                for (j = imageSize.heightStart(); j <= imageSize.heightEnd(); j++) {
                    ...
                    if (color.getRed() <= this.inputFilterValue)
                        buffImage.setRGB(i, j, 256 * color.getGreen() + color.getBlue());
                }

            if (this.segmentVlaue == 16) {
                while (ImageFilterRed.nrOfThreadsDone < 15)
                    try {
                        Thread.sleep(10);
                    } catch (InterruptedException e) {
                        System.out.println("Error: Thread.sleep(10) (ImageFilterRed)");
                        e.printStackTrace();
                    }
                end = (int) System.currentTimeMillis();
                ImageWriterClass imageWriterClass = new ImageWriterClass(imagePaths, buffImage, end - start, "ImageFilterRed", String.valueOf(this.inputFilterValue), imageView);
                imageWriterClass.writeImage();
            }
            end = (int) System.currentTimeMillis();
            this.setExecTime(end - start);
            ImageFilterRed.nrOfThreadsDone++;
        } catch (IOException e) {
