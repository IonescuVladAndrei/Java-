...
public class ImageFilterInvertByX extends Thread implements ImageFilter {
    ...
    public ImageFilterInvertByX(BufferedImage buffImage, ImagePaths imagePaths, int segmentVlaue, ImageView imageView) throws IOException {
        ...
        try {
            ...
            for (i = imageSize.widthStart(); i <= imageSize.widthEnd(); i++)
                for (j = imageSize.heightStart(); j <= imageSize.heightEnd(); j++) {
                    ...
                    buffImage.setRGB(i, buffImage.getHeight() - j - 1, color.getRGB());
                }
            if (this.segmentVlaue == 16) {
                while (ImageFilterInvertByX.nrOfThreadsDone < 15)
                    try {
                        Thread.sleep(10);
                    } catch (InterruptedException e) {
                        System.out.println("Error: Thread.sleep(10) (ImageFilterInvertByX)");
                        e.printStackTrace();
                    }
                    end = (int) System.currentTimeMillis();
                    ImageWriterClass imageWriterClass = new ImageWriterClass(imagePaths, buffImage, end - start, "ImageFilterInvertByX", "-", imageView);
                    imageWriterClass.writeImage();
            }
            end = (int) System.currentTimeMillis();
            this.setExecTime(end - start);
            ImageFilterInvertByX.nrOfThreadsDone++;
        } catch (IOException e) {
