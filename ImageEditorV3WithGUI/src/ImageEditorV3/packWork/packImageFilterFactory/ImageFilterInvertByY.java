...
public class ImageFilterInvertByY extends Thread implements ImageFilter {
    ...
    public ImageFilterInvertByY(BufferedImage buffImage, ImagePaths imagePaths, int segmentVlaue, ImageView imageView) throws IOException {
        ...
        try {
            ...
            for (i = imageSize.widthStart(); i <= imageSize.widthEnd(); i++)
                for (j = imageSize.heightStart(); j <= imageSize.heightEnd(); j++) {
                    ...
                    buffImage.setRGB(buffImage.getWidth() - i - 1, j, color.getRGB());
                }
            if (this.segmentVlaue == 16) {
                while (ImageFilterInvertByY.nrOfThreadsDone < 15)
                    try {
                        Thread.sleep(10);
                    } catch (InterruptedException e) {
                        System.out.println("Error: Thread.sleep(10) (ImageFilterInvertByY)");
                        e.printStackTrace();
                    }
                end = (int) System.currentTimeMillis();
                ImageWriterClass imageWriterClass = new ImageWriterClass(imagePaths, buffImage, end - start, "ImageFilterInvertByY", "-", imageView);
                imageWriterClass.writeImage();
            }
            end = (int) System.currentTimeMillis();
            this.setExecTime(end - start);
            ImageFilterInvertByY.nrOfThreadsDone++;
        } catch (IOException e) {
