...
public class ImageFilterSat extends Thread implements ImageFilter {
    ...
    public ImageFilterSat(BufferedImage buffImage, ImagePaths imagePaths, int segmentVlaue, int inputFilterValue, ImageView imageView) {
        ...
        try {
            ...
            for (i = imageSize.widthStart(); i <= imageSize.widthEnd(); i++)
                for (j = imageSize.heightStart(); j <= imageSize.heightEnd(); j++) {
                    ...
                    if (this.inputFilterValue == -255)
                        this.inputFilterValue++;
                    else if (this.inputFilterValue == 255)
                        this.inputFilterValue--;
                    alpha = (double) (255 + this.inputFilterValue) / (255 - this.inputFilterValue);
            ...
            if (this.segmentVlaue == 16) {
                while (ImageFilterSat.nrOfThreadsDone < 15)
                    try {
                        Thread.sleep(10);
                    } catch (InterruptedException e) {
                        System.out.println("Error: Thread.sleep(10) (ImageFilterSat)");
                        e.printStackTrace();
                    }
                end = (int) System.currentTimeMillis();
                ImageWriterClass imageWriterClass = new ImageWriterClass(imagePaths, buffImage, end - start, "ImageFilterSat", String.valueOf(this.inputFilterValue), imageView);
                imageWriterClass.writeImage();
            }
            end = (int) System.currentTimeMillis();
            this.setExecTime(end - start);
            ImageFilterSat.nrOfThreadsDone++;
        } catch (IOException e) {
