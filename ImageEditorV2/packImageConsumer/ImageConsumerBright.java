...
public class ImageConsumerBright extends Thread implements ImageConsumer{
	...
	public ImageConsumerBright(ImageBuffer b, int position, ImagePath inputPath, int filterValue,DataOutputStream out) {
		this.buffer = b;
		...
		try {
			...
			for (i = widthStart; i <= widthEnd; i++) {
				for (j = heightStart; j <= heightEnd; j++) {
					...
					R=this.overLimit(color.getRed()+this.filterValue);
                	G=this.overLimit(color.getGreen()+this.filterValue);
                	B=this.overLimit(color.getBlue()+this.filterValue);
					out.writeInt(R);
					out.writeInt(G);
					out.writeInt(B);
