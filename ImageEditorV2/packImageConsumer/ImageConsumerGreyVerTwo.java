...
public class ImageConsumerGreyVerTwo extends Thread implements ImageConsumer{
	...
	public ImageConsumerGreyVerTwo(ImageBuffer b, int position, ImagePath inputPath, int filterValue,DataOutputStream out) {
		this.buffer = b;
	...
	public void run() {
		...
		int i, j, widthStart, widthEnd, heightStart, heightEnd, R, G, B, gs;
		...
		try {
			...
			for (i = widthStart; i <= widthEnd; i++) {
				for (j = heightStart; j <= heightEnd; j++) {
					...
					gs = (int) (0.299*color.getRed()+0.587*color.getGreen() +0.114*color.getBlue());
                	R = G = B = gs;
					out.writeInt(R);
					out.writeInt(G);
					out.writeInt(B);
