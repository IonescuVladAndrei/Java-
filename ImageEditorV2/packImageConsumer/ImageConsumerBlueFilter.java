...
public class ImageConsumerBlueFilter extends Thread implements ImageConsumer{
	...
	public ImageConsumerBlueFilter(ImageBuffer b, int position, ImagePath inputPath, int filterValue,DataOutputStream out) {
		this.buffer = b;
		...
		try {
			...
			for (i = widthStart; i <= widthEnd; i++) {
				for (j = heightStart; j <= heightEnd; j++) {
					...
					if(color.getBlue() <= this.filterValue)
                        out.writeInt(0);
                    else
