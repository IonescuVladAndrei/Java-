		...
		while (inputCommand.compareTo("S") != 0) {
			...
			if (inputCommand.equals("S"))
				...
				if(inputCommand.equals("Y"))
					System.out.println("Lista filtre:\nNegare -> 1\nGrey V1 -> 2\nGrey V2 -> 3\nLuminozitate -> 4\nSaturatie -> 5\nGama -> 6\nSepia -> 7\nContrast -> 8\nOglindire -> 9\nPictura(pentru imagini pana in ~ 900x600 !!!) -> 10\nFiltrare rosu -> 11\nFiltrare verde -> 12\nFiltrare albastru -> 13");
				inputCommand = "0";
				while (Integer.parseInt(inputCommand) < 1 || Integer.parseInt(inputCommand) > 13) {
					System.out.println("Introduceti ID-ul filtrului (1-13)");
					inputCommand = reader.readLine();
					System.out.println("Ati introdus ID-ul: " + inputCommand);
				}
				filter = Integer.parseInt(inputCommand);
				if(filter > 3 && filter < 7 || filter == 8 || filter > 10 && filter < 14 ) {
					inputCommand = "256";
