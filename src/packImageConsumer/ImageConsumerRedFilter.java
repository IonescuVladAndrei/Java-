...
public class ImageConsumerRedFilter extends Thread implements ImageConsumer{
	...
	public ImageConsumerRedFilter(ImageBuffer b, int position, ImagePath inputPath, int filterValue,DataOutputStream out) {
		this.buffer = b;
		...
		try {
			...
			for (i = widthStart; i <= widthEnd; i++) {
				for (j = heightStart; j <= heightEnd; j++) {
					...
					if(color.getRed() <= this.filterValue)
                    out.writeInt(0);
                    else
